...
fetch_sha256() {
    ...
    local url="https://packages.gitlab.com/gitlab/gitlab-ee/packages/debian/${debian_version}/gitlab-ee_${version}-ce.0_${architecture}.deb"
    ...
    for debian_version in "${debian_versions[@]}"; do
        ...
        if ! grep -q "\[resources\.sources\.latest_${debian_version}\]" "$manifest_file"; then
            ...
            if [ -n "$last_latest_line" ]; then
                ...
                # Insert debian version section
                {
                    ...
                    echo "            extract = false"
                    echo "            rename = \"gitlab-ee.deb\""
                    for arch in "${architectures[@]}"; do
                        echo "            ${arch}.url = \"https://packages.gitlab.com/gitlab/gitlab-ee/packages/debian/${debian_version}/gitlab-ee_${version}-ce.0_${arch}.deb/download.deb\""
        ...
        for arch in "${architectures[@]}"; do
            sed -i "/\[resources\.sources\.latest_${debian_version}\]/,/\[resources\.sources\./ {
                s|${arch}\.url = \"https://packages.gitlab.com/gitlab/gitlab-ee/packages/debian/${debian_version}/gitlab-ee_[^\"]*\"|${arch}.url = \"https://packages.gitlab.com/gitlab/gitlab-ee/packages/debian/${debian_version}/gitlab-ee_${version}-ce.0_${arch}.deb/download.deb\"|
...
if [ "$add_only" = false ]; then
    ...
    url="https://gitlab.com/gitlab-org/omnibus-gitlab/-/raw/${version}+ee.0/files/gitlab-config-template/gitlab.rb.template"
